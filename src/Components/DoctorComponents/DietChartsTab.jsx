...
const DietChartsTab = () => {
  ...
  const statusFilters = [
    { value: "all", label: "All Charts", count: dietCharts.length, color: "bg-gray-500" },
    { value: "active", label: "Active", count: dietCharts.filter(c => c.status === "active").length, color: "bg-green-500" },
    { value: "pending", label: "Pending Review", count: dietCharts.filter(c => c.status === "pending").length, color: "bg-amber-500" },
    { value: "completed", label: "Completed", count: dietCharts.filter(c => c.status === "completed").length, color: "bg-blue-500" }
  ];
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl p-4 border border-amber-100">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <select className="px-4 py-2 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent">
          ...
          <div className="flex items-center gap-2 text-sm text-amber-700">
            ...
            <span>{filteredCharts.length} diet charts found</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredCharts.map((chart) => (
          <div key={chart.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden">
            {/* Chart Header */}
            <div className="p-4 border-b border-gray-100">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className={getPriorityBadge(chart.priority)}>{chart.priority} priority</span>
                    </div>
                  </div>
                </div>
                <button 
                ...
                >
                  <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${expandedChart === chart.id ? 'rotate-180' : ''}`} />
                </button>
              </div>
            ...
            <div className="p-4">
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="text-center p-3 bg-gray-50 rounded-lg">
                  <div className="text-2xl font-bold text-gray-900">{chart.duration}</div>
                  <div className="text-xs text-gray-600">Duration</div>
                </div>
                <div className="text-center p-3 bg-gray-50 rounded-lg">
                  <div className={`text-2xl font-bold ${getComplianceColor(chart.compliance)}`}>
                    {chart.compliance}%
                  </div>
              ...
              <div className="border-t border-gray-100 p-4 bg-gray-50">
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-2 text-sm">
                    {Object.entries(chart.meals).map(([meal, description]) => (
                      <div key={meal} className="flex justify-between items-center p-2 bg-white rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    <h4 className="font-semibold text-gray-900 mb-2">Supplements</h4>
                    <div className="flex flex-wrap gap-2">
                ...
                <div className="mb-4">
                  ...
                  <p className="text-sm text-gray-600 bg-white p-3 rounded-lg">{chart.notes}</p>
                </div>

                {/* Action Buttons */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <button className="flex items-center gap-2 bg-amber-500 text-white px-3 py-2 rounded-lg text-sm hover:bg-amber-600 transition-colors">
