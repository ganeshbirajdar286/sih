...
import { FaFilePdf, FaFileMedical, FaDownload, FaEye, FaShare, FaCalendarAlt, FaFilter, FaSearch, FaChartLine, FaPrint } from 'react-icons/fa';
...
const Reports = () => {
  ...
  const reportTypes = [
    { id: 'all', name: 'All Reports', icon: <FaFileMedical />, count: medicalReports.length },
    { id: 'lab', name: 'Lab Reports', icon: <FaFileMedical />, count: medicalReports.filter(r => r.type === 'Lab Report').length },
    { id: 'imaging', name: 'Imaging', icon: <FaFileMedical />, count: medicalReports.filter(r => r.type === 'Imaging Report').length },
    { id: 'diagnostic', name: 'Diagnostic', icon: <FaChartLine />, count: medicalReports.filter(r => r.type === 'Diagnostic Report').length },
  ];
  ...
  const filteredReports = medicalReports.filter(report => {
    ...
    if (activeTab === 'diagnostic') return report.type === 'Diagnostic Report';
    return true;
  }).filter(report => 
  ...
  const getTypeIcon = (type) => {
    switch (type) {
      case 'Lab Report': return <FaFileMedical className="text-blue-500" />;
      case 'Imaging Report': return <FaFileMedical className="text-purple-500" />;
      case 'Pathology Report': return <FaFileMedical className="text-red-500" />;
      case 'Diagnostic Report': return <FaChartLine className="text-yellow-500" />;
      case 'Annual Physical': return <FaFileMedical className="text-green-500" />;
      case 'Specialist Report': return <FaFileMedical className="text-indigo-500" />;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">Access and manage your medical test results and reports</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          {reportTypes.map(type => (
            <div 
              key={type.id} 
              className={`bg-white p-5 rounded-xl shadow-sm cursor-pointer transition hover:shadow-md ${activeTab === type.id ? 'border-l-4 border-green-500' : ''}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredReports.length > 0 ? (
            filteredReports.map(report => (
              <div key={report.id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition overflow-hidden">
                <div className="p-6">
                  <div className="flex justify-between items-start">
                    <div className="flex items-start">
                      ...
                      <div>
                        ...
                        <p className="text-gray-600">{report.type} • {report.doctor}</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-lg max-w-4xl w-full max-h-screen overflow-y-auto">
            <div className="p-6 border-b border-gray-200">
              ...
              <p className="text-gray-600 mt-1">{selectedReport.type} • {selectedReport.doctor}</p>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 text-gray-900">{formatDate(selectedReport.date)}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">File Type:</span>
                    <span className="ml-2 text-gray-900">{selectedReport.fileType.toUpperCase()}</span>
                  </div>
                  <div>
              ...
              <div className="bg-gray-800 text-white p-4 rounded-lg mb-6 text-center">
                <div className="flex justify-center items-center mb-4">
                  ...
                  <div>
                    <p className="font-medium">This is a preview of your report</p>
                    <p className="text-sm text-gray-300">The full report is available for download</p>
                  </div>
                </div>
